...
class AttributionTask:
    ...
    def __init__(
        ...
        if self.batch_mode:
            ...
            grad(loss_func),               # 输入: (params, ((image, text), index))
            ...
            print("Use batch in loss and target func!")
        else:
            self.grad_loss_func = vmap(
                grad(self.loss_func),
                in_dims=(None, 1),
    ...
    def get_grad_target_func(
        ...
        if self.grad_target_func_kwargs != grad_target_func_kwargs:
            if self.batch_mode:
                self.grad_target_func = vmap(
                grad(target_func),               # 输入: (params, ((image, text), index))
                in_dims=(None, (None, 0)),              # 控制 param 不变，(image, text) batch 化，index 单个提取
                randomness="different",
            )
                print("Use batch in target func!")
            else:
                self.grad_target_func = vmap(
                    grad(target_func),
                    in_dims=in_dims,
